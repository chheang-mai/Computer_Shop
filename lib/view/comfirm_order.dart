import 'package:computer_shop/view/payment_screen.dart';
...
class _ConfirmOrderState extends State<ConfirmOrder> {
  ...
  Future<void> _navigateToLocationPicker() async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => LocationPicker(paymentData: paymentData),
      ),
    );

    if (result != null && mounted) {
      setState(() {
        paymentData.selectedAddress = result["address"];
        paymentData.selectedLatLng = LatLng(result["lat"], result["lng"]);
        paymentData.selectedShop = result["shop"];
        paymentData.distanceToShop = result["distance"] ?? 0.0;
        paymentData.deliveryTime = result["duration"] ?? 0;
      });
    }
  }
    ...
    return Scaffold(
            ...
            Padding(
              ...
              child: Column(
                children: [
                  GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          color: paymentData.selectedAddress.isNotEmpty
                              ? Colors.blue[700]!
                              : Colors.grey[300]!,
                          width: paymentData.selectedAddress.isNotEmpty ? 2 : 1,
                        ),
                        boxShadow: [
                          BoxShadow(
                            blurRadius: paymentData.selectedAddress.isNotEmpty ? 12 : 8,
                            spreadRadius: 0,
                            offset: Offset(0, 4),
                            color: Colors.black.withOpacity(
                                paymentData.selectedAddress.isNotEmpty ? 0.15 : 0.08),
                          )
                        ],
                      ),
                      child: Column(
                          ...
                          Row(
                            children: [
                              Container(
                                ...
                                child: Icon(
                                  paymentData.selectedAddress.isNotEmpty
                                      ? Icons.check_circle
                                      : Icons.add_location,
                                  color: paymentData.selectedAddress.isNotEmpty
                                      ? Colors.blue[700]
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      ...
                                      style: TextStyle(
                                        fontSize: paymentData.selectedAddress.isNotEmpty ? 15 : 16,
                                        fontWeight: paymentData.selectedAddress.isNotEmpty
                                            ? FontWeight.w500
                                            : FontWeight.w400,
                                        color: paymentData.selectedAddress.isNotEmpty
                                            ? Colors.black87
                  ...
                  if (paymentData.selectedLatLng != null)
                    Container(
                      margin: EdgeInsets.only(top: 16),
                      height: 200,
                      decoration: BoxDecoration(
                      ...
                      child: GoogleMap(
                        initialCameraPosition: CameraPosition(
                          target: paymentData.selectedLatLng!,
                          zoom: 13,
                        ),
                        markers: {
                          Marker(
      ...
      bottomNavigationBar: Container(
        ...
        child: ElevatedButton(
          onPressed: paymentData.selectedAddress.isNotEmpty
  ...
  Widget _buildShopInfo() {
    Shop shop = paymentData.selectedShop!;
    double distance = paymentData.distanceToShop;
    int deliveryTime = paymentData.deliveryTime > 0
        ? paymentData.deliveryTime
        : (distance * 5).ceil();
          ...
          children: [
            ...
            Expanded(
              child: Row(
                ...
                children: [
                  Icon(Icons.schedule, size: 20, color: Colors.green[700]),
                  SizedBox(width: 8),
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
...
class LocationPicker extends StatefulWidget {
  ...
  const LocationPicker({super.key, required this.paymentData});
...
class _LocationPickerState extends State<LocationPicker> {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'OK') {
          ...
          setState(() {
            ...
            routeDuration =
                data['routes'][0]['legs'][0]['duration']['value'] ~/ 60; // minutes
          });

          print(
              '✅ Route loaded: ${routeCoordinates.length} points, ${routeDistance.toStringAsFixed(1)}km, ${routeDuration}min');
  ...
  void _fallbackToStraightLine(LatLng origin, LatLng destination) {
    setState(() {
      ...
      routeDistance = widget.paymentData.calculateDistance(origin, destination);
      ...
      SnackBar(
        content: Row(
          children: [
            ...
            SizedBox(width: 12),
            Expanded(
              child: Text('Please enable Directions API in Google Cloud Console'),
            ),
          ],
        ),
  ...
  void _updateMarkersWithRoute() {
    ...
    // Add shop markers
    for (var shop in widget.paymentData.shops) {
  ...
  void _findNearbyShops(LatLng userLocation) {
    nearbyShops = widget.paymentData.getShopsWithDistance(userLocation);
    if (nearbyShops.isNotEmpty) {
      nearestShop = nearbyShops.first['shop'];
      widget.paymentData.distanceToShop = nearbyShops.first['distance'];
    }
  }
  ...
  void _showShopDetails(Shop shop) {
    ...
    if (shop == nearestShop && routeDistance > 0) {
      ...
      distance = widget.paymentData.calculateDistance(selectedPoint, shop.location);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title:
            Text("Select Location", style: TextStyle(fontWeight: FontWeight.w600)),
        elevation: 0,
        actions: [
          IconButton(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      leading: Container(
                        ...
                        decoration: BoxDecoration(
                          color:
                              isNearest ? Colors.green[50] : Colors.grey[100],
                          borderRadius: BorderRadius.circular(8),
                        ),
                        child:
                            Text(shop.image, style: TextStyle(fontSize: 24)),
                      ),
                      title: Row(
                        children: [
                          Text(shop.name,
                      ...
                      subtitle: Text(
                          '${distance.toStringAsFixed(1)} km • $time min'),
                            ...
                            children: [
                              Text(
                                routeDistance > 0
                                    ...
                                    : '${widget.paymentData.distanceToShop.toStringAsFixed(1)} km',
                                style: TextStyle(
                                  fontSize: 14,
                                  fontWeight: FontWeight.bold,
                                  color: Colors.green[700],
                          ...
                          onPressed: () {
                            Navigator.pop(context, {
                              ...
                              "distance": routeDistance > 0
                                  ? routeDistance
                                  : widget.paymentData.distanceToShop,
